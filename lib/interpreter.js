...
function forLoopStatementHandler(node, scope) {
  var userVar = node.val[0].val;
  var start = node.val[1].val[0],
      end = node.val[1].val[1];
  var loopBody = node.val[2];

  for (var i = start; i <= end; i++) {
    setInScope(scope, userVar, i);
    loopBody.forEach(function(n) {
      interpret(n, scope);
    });
  }
}
...
function interpret(rootNode, scope) {
  ...
  switch (rootNode.type) {
    ...
    case "ForloopStatement":
      return forLoopStatementHandler(rootNode, scope);
